...
p.~\pageref{nilm:emi}), sendo necessário encontrar características
suficientemente discriminantes. No entanto, utiliza-se o termo
\glspl{c6} exclusivamente para identificar os casos em que a
...
F^i=2\dfrac{\dfrac{N_{id}}{N_{id}+N_{fp}}\eta_{total}}
{\dfrac{N_{id}}{N_{id}+N_{fp}}+\eta_{total}}
\end{equation}
...
\item Enfim, é levantada a estatística detalhando o
...
Também com o objetivo de possibilitar a saturação do método, mas no
...
com frequência maior, aumentando as ocorrências de erros devido à
\gls{c6}. O autor criou o método \gls{vast} para empregar a informação
...
em 20 \gls{hz}. Devido às dificuldades para etiquetar os momentos
...
sintetizar a informação contínuos na decomposição do sinal pela
\gls{tw}. As características utilizadas são os
picos para os níveis de detalhes do Método de Burg adicionados do
\gls{fp}.
...
casos empilhados, porém o mesmo foi bastante trivial, apenas
adicionando uma lâmpada incandescente antes de acionar os outros
equipamentos. A resposta mostrou que o grupo de equipamentos com
motores sofreu deterioração de sua eficácia. Assim, não é possível
deduzir a capacidade do método para a aplicação em condições reais,
onde é necessário a robustez das técnicas aplicadas para o
empilhamento de diversos equipamentos, injetando ruídos e dificultando
...
Monte-Carlo, capaz de gerar situações de empilhamento de aparelhos,
...
central. A ideia é reduzir o empilhamento de informação colocando os
medidores mais próximos ao consumo, facilizando assim a identificação
dos aparelhos. Cada módulo de submedição realiza medições em
3,2~k\acs{hz} de tensão e corrente, que são utilizados para gerar
a \acl{p}, \acl{q} e os harmônicos impares de corrente com
indices de 1-11. Quando é detectado um evento de transição, a
informação é repassada para uma central, que procura no catálogo o
aparelho que melhor se corresponde com a transição. Reporta-se
eficiência de detecção de 95\% para submedidores com um único
aparelho. 
...
desconhecidos, bem como de \gls{c5}, que conforme o seu empilhamento
irão tornar a rede um ambiente muito mais complexo para operação das
técnicas. É justamente nesse ambiente que as abordagens devem ser
testadas, demonstrando como será o seu comportamento nas condições
adversas presentes nas redes elétricas residenciais.
...
três vertentes. Buscando a saturação do método, uma vertente
...
dessa abordagem seria como etiquetar os modelos encontrados com os
...
empilhamento de aparelhos que intrinsecamente torna a tarefa mais
...
haverá maior operação, causando o empilhamento de diversos aparelhos
que injetarão ruídos na rede.  Devido a presença de aparelhos
deconhecidos, um aspecto desejável para o \gls{nilm} é conseguir
detectá-los de maneira automática e de algum modo possibilitar a
correlação dos modelos identificados com seus nomes. Outra questão é
como obter informação discriminante garantindo que o projeto seja
viável. A utilização de amostragem baixa deteriora a capacidade de
discriminação, por outro lado é possível que a estrutura para essas
abordagens já esteja disponível. Essa abordagem parece ser a mais
próxima de tornar-se realidade. Enquanto a elevação da amostragem
aumenta a eficiência, dificulta a aplicação do projeto por necessitar
de \emph{hardware} específico. Além disso, as técnicas presentes
atualmente para alta amostragem ainda precisam ser testadas quanto a
sua robustez. Todavia, um outro caminho para obter maiores
...
colaborações realizadas pela \acs{coppe} no mesmo. Os trabalhos
de autores precedentes ao presente trabalho é realizado na
...
Subsessão~\ref{ssec:met_cepel}, sendo essa a realidade onde o trabalho
atual auxiliou. Na Subsessão~\ref{ssec:caracteristicas} é realizada
considerações quanto a escolha da \gls{fex} em relação a
...
mais extensa.} onde a extração da envoltória utilizando a decomposição
...
mais de 1~\acs{a}\footnote{\citeauthor*{nilm_cepel_bezerra}
explicita em seu texto que houve melhorias quanto a essas limitações
no seu sistema de aquisição, no entanto não foi possível encontrar o
novo valor para esse limiar.}. Houve também uma melhoria no levantamento do
banco de dados, havendo uma amostragem bastante superior aos trabalhos
anteriores, com um total de 1324 acionamentos observados. O trabalho
implementou dois detectores especialistas para a identificação dos
momentos de acionamentos (a Etapa~\ref{itm:etapa2}, porém apenas para
acionamentos), e um baseado em correlação. Foram utilizadas 150
amostras da envoltória de corrente, obtendo taxa de detecção de 86~\%.
Grande parte da inexatidão ocorreu devido aos erros no grupo de
eletrônicos, obtendo taxas de classificação bastante inadequadas para
esse grupo, na ordem de 30~\%.
...
a maior dificuldade para os \glspl{nilm} ocorrem devido ao
empilhamento de equipamentos na rede, em especial devido à decorrência
de \gls{c5}. Os esforços para realizar a Etapa~\ref{itm:etapa2}
envolveram apenas a detecção de acionamentos, bem como não fica claro
no trabalho de \cite{nilm_cepel_alvaro} a maneira proposta para tratar
a questão de classificação do desligamento. Indo adiante, a taxonomia
...
% TODO Colocar aqui uma referencia para uma figura mais a frente.
...
%configurações mais simples, havendo empilhamento em configurações
%simples e dados limpos. 

A fim de avaliar a eficácia, o relatório utilizou de novos dados, onde
algumas configurações já apresentavam empilhamento de aparelhos
...
Em especial quanto ao terceiro item, há um interesse por parte do
\gls{cepel} de procurar uma solução menos complexa, mesmo
que isso deteriore a capacidade do \gls{nilm}, mas que resulte em uma
resposta mais rápida em tempo de projeto. Por esse mesmo motivo, o
\gls{cepel} considera a utilização de medidores presentes no mercado,
dando preferência para os de baixo custo para facilitar o projeto, já
que o desenvolvimento do \emph{hardware} adiciona peso e reduz a
velocidade do projeto. Por enquanto, dois medidores estão sendo
utilizados, um proprietário do \acs{cepel} e um medidor da empresa
\emph{Yokogawa} \cite{yokogawa_medidor}, estando alguns dados
disponíveis em uma configuração, e outros na outra.

As variáveis anteriormente citadas são as que estão disponíveis
atualmente para serem utilizadas (em amostragem de 60~\acs{hz}).
Este trabalho não irá entrar na questão da discriminação,
sendo realizadas considerações na subsessão a seguir para auxiliar na
escolha das características a serem empregadas. Como é possível
...
Porém, como foi dito na Subsessão~\ref{ssec:met_cepel}, ainda não foi
definido o caminho para qual vertente o projeto irá seguir, havendo
uma tendência para seguir o caminho de baixa amostragem. Nesse caso,
terá acesso apenas a variáveis macroscópicas, como \acs{di}, \acs{dp}
...
medidor utilizado. Se optar por alta amostragem, será necessário
explorar frequências maiores à 60~\acs{hz} para obter informação
...
a uma única dessas características, podendo abranger-se para um
conjunto delas ou aplicar/adicionar uma característica ainda não
explorada para obter melhor capacidade discriminativa. Nessa
alternativa, pode-se também explorar a envoltória das ondas já que
...
o acionamento. Essa capacidade possivelmente poderá auxiliar para as
...
nilm_zeifman_statistical_vastext_1stws_2012,
nilm_zeifman_vast_hisample_pdfmerge_2011,
nilm_zeifman_statistical_naive_enduses_2013}. Finalmente, é possível
manter a mente aberta para a possibilidade de utilizar submedidores,
seja para reduzir o empilhamento dos aparelhos a uma quantidade
aceitável, ou para obter diretamente a informação desagregada de
aparelhos complexos e importantes em termos de uso-final
\cite{seminilm_ihome_tomek_2012,seminilm_fhmm_empiricalnmeter_2013,
seminilm_berges_multisensor_2010}.
